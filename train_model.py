...
cnn_optimizer = optim.Adam(mario_cnn.parameters(), lr=1e-2)
...
for i_episode in range(1, num_episodes + 1):
    ...
    for t in range(max_t):
        ...
        assert predicted_features.shape == next_state_features.shape, f"Shape mismatch: {predicted_features.shape} != {next_state_features.shape}"
        ...
        if done:
            print(f"Episode {i_episode} - Score: {score} - Eps: {eps:.2f} - CNN Loss: {cnn_loss:.4f} - DQN Loss: {dqn_loss:.4f}")
            break
